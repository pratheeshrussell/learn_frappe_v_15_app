...
frappe.views.EmployeeimageView = class EmployeeImageView extends (
    ...
    info_fields.forEach((field, index) => {
      if (item[field] && !set) {
        if (field === "status") {
          ...
          info_html += `<li><span class="emp-status-${statusClass}">${__(
            item[field]
          )}</span></li>`;
...
frappe.views.ListViewSelect = class CustomListViewSelect extends (
  frappe.views.ListViewSelect
) {
  constructor(opts) {
    super(opts);
  }
  setup_views() {
    const views = {
      ...
      Inbox: {
        condition:
          this.doctype === "Communication" && frappe.boot.email_accounts.length,
        action: () => this.set_route("inbox"),
        current_view_handler: () => {
          const accounts = this.get_email_accounts();
          let default_action;
          if (
            has_common(frappe.user_roles, ["System Manager", "Administrator"])
          ) {
            default_action = {
